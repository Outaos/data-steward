...
staff_col = "GIS Staff Assigned"

required_cols = [title_col, category_col, staff_col]
...
df[staff_col] = df[staff_col].fillna("Unassigned").astype(str).str.strip()
df.loc[df[staff_col] == "", staff_col] = "Unassigned"
...
def classify_title(title: str) -> str:
    ...
    # Fallback: CP000 / CP 000 only if nothing else matched
...
# =========================
# SUMMARY BY GROUP + GIS STAFF
# =========================
staff_summary = pd.crosstab(df["Title_Group"], df[staff_col])

# Add total column
staff_summary["Total"] = staff_summary.sum(axis=1)

# Convert index to column
staff_summary = staff_summary.reset_index()

# Add percent column
grand_total = staff_summary["Total"].sum()
staff_summary["Percent"] = (staff_summary["Total"] / grand_total * 100).round(1)

# Optional nicer display
staff_summary["Percent"] = staff_summary["Percent"].astype(str) + "%"

# Reorder columns: Group, Total, Percent, then staff columns
base_cols = ["Title_Group", "Total", "Percent"]
staff_cols = [col for col in staff_summary.columns if col not in base_cols]
staff_cols = sorted([col for col in staff_cols if col != "Percent" and col != "Total"])

staff_summary = staff_summary[["Title_Group", "Total", "Percent"] + staff_cols]

# Sort by total descending
staff_summary = staff_summary.sort_values("Total", ascending=False).reset_index(drop=True)

# Rename first column
staff_summary = staff_summary.rename(columns={"Title_Group": "Group"})

print("\n📊 Summary by Group and GIS Staff Assigned:")
print(staff_summary.to_string(index=False))
...
summary_path = output_folder / "Clearance_pattern_summary_by_gis_staff.csv"
other_path = output_folder / "Clearance_pattern_other_rows.csv"

staff_summary.to_csv(summary_path, index=False)
